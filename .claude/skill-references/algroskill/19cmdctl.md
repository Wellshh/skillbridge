...
`axlCmdRegister( t_allegroCmd ts_callback ?cmdType t_cmdType ?doneCmd ts_doneCmd ?cancelCmd ts_cancelCmd ?undo t ) => t/nil`
...
| `undo` | If `t`, command can be undone. It is only applicable for `t_cmdType "interactive"`. Refer Tip section for programming requirements of undo commands.<br><br>Other options may be added at a future time. |
...
`mypath = axlPathStart( list(8900:4400))``axlPathArcRadius(mypath, 12., 8700:5300, nil, nil, 500)``myline = axlDBCreatePath( mypath, "etch/top" nil)``; Arc is not yet visible``axlFlushDisplay()``; Now arc is visible`
...
`axlBuildClassPopup( r_form t_field ) => t/nil`
...
`axlBuildSubclassPopup( r_form t_field t_class ) => t/nil`
...
`axlSubclassFormPopup( r_form t_field t_class nil/lt_subclass ) => t/nil`
...
| Name | Description |
|---|---|
| `r_form` | Standard form handle (see axlFormCreate ) |
| `t_field` | Field name in form or pop-up name of form. |
| `t_class` | Class name. |
