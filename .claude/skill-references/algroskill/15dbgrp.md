...
`axlDBAddGroupObjects( o_group lo_members ) => t/nil`
...
| `t_type` | String defining the group type. Legal values are: 
 ...
 members: xnet, net, diffpair bus, net_group shape and fill rects (via and route keepout)
 ...
 "`module`" - suggest to use `axlDBCreateModuleDef` Accessible but not recommended.
 "`db_drill_legend`" - drill legend tables
 "`rf_nets`" - RF "super" nets |
| `lo_members` | List of AXL `dbids` defining the group members. Duplicate `dbid` entries are silently ignored. An object is added to a group only once. A single `dbid` can be substituted for a list.If certain restrictions on the group members are violated, this function fails and returns `nil`. 
...
| `g_mode` | If this value is `t`, it promotes the object to its net/xnet and performs test on that object.
...
Note: All restrictions and disclaimers specified in axlDBCreateGroup also apply for this API.
