...
part: 2/2
estimated_tokens: 10776
-->

### axlFormColorize

`axlFormColorize( o_form t_field g_option g_color ) => t/nil`
...
- If setting the background of a ENUM field on Windows, you must set the OPTION color in the form file as in the following example.`FIELD enum_control``FLOC 16 12``ENUMSET 41``OPTIONS color``POP "enum_popup"``ENDFIELD`This is the same option as the '`ownerdrawn`'. The color option is ignored on UNIX which does not support themes. For theming purposes, Microsoft takes the background color so setting this option disables Microsoft themes for this control.
...
`axlFormGridCancelPopup( r_form t_field ) => t/nil`
...
`axlFormGridDeleteRows( r_form t_field x_row x_number ) => t/nil`
...
`axlFormGridEvents( r_form t_field s_event/(s_event1 s_event2 ...) ) => t/nil`
...
| `'mrowselect` | Puts grid into multi-row select mode. This is mutually exclusive with `cellselect`. Use axlFormGridSelected to determine what rows are selected. |
...
`axlFormGridGetCell( r_form t_field r_cell ) => r_cell/nil`
...
`axlFormGridInsertCol( r_form t_field r_formGridCol ) => t/nil`
...
Note: For more information on using this function, see Using Grids for an overview.
...
`axlFormGridInsertRows( r_form t_field x_row x_number ) => t/nil`
...
`axlFormGridReset( r_form t_field ) => t/nil`
...
`axlFormGridSetBatch( r_form t_field s_callback g_pvtData ) => t/nil`
...
`axlFormGridUpdate( r_form t_field ) => t/nil`
...
`axlFormInvalidateField( r_form t_field ) => t/nil`
...
`axlFormIsFieldEditable( r_form t_field ) => t/nil`
...
`axlFormListAddItem( r_form t_field t_listItem/lt_listItems/nil g_index ) => t/nil`
...
`axlFormListGetSelCount( r_form t_field ) => x_count/nil`
...
`axlFormListSelAll(fw "mlistfield" t)` De-Select all items in multi-list control; `mlistfield`.
...
`axlFormListSelAll(fw "mlistfield" nil)` Retrieves and prints a message defined in the form file by message label (`t_messageLabel`.) Form file allows definitions of messages using the "`MESSAGE`" keyword (see Using Forms Specification Language.) Use this to give a user access to message text, but no access to your SKILL code.
...
Returns the control type for a form field. See the keywords in Callback Procedure: formCallback for a list of supported field types.
...
`axlFormSetActiveField( r_form t_field ) => t/nil`
...
`axlFormSetDecimal( o_form g_field x_decimalPlaces ) => t/nil`
...
`axlFormSetFieldEditable( r_form t_field g_editable ) => t/nil`
...
`axlFormSetFieldLimits( o_form t_field g_min g_max ) => t/nil`
...
`axlFormTreeViewChangeLabel( r_form t_field g_hItem t_label ) => t/nil`
...
`tree = axlFormTreeViewAddItem(fw "tree" "one" nil nil 'TVSELECT_3STATE) l = axlFormTreeViewLoadBitmaps(fw "tree" list("myBmp1" "myBmp2")) axlFormTreeViewChangeImages(fw "tree" tree car(l) cadr(l) nil)` axlFormTreeViewLoadBitmaps may return (6 7 8) into variable l.
...
- If nil is passed for TV_EXPAND or TV_COLLAPSE then all levels (including) children are expanded or collapse.
...
`axlFormTreeViewSetSelectState( r_form t_field g_hItem g_state ) => t/nil`
