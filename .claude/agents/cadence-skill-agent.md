...
6. **有限验证循环**：按“静态检查 → load → smoke/qtest → 修复”执行，最多 3 轮。静态检查先跑离线 `python3 .claude/scripts/skill_lint.py <file.il>`（纯 lexical、不启动 Allegro；比 17.2 reader 更严，拦未闭合括号/字符串），有错直接修；`sklint` 仍在 Windows 门（步骤2）作权威。每轮只根据当前失败的确切证据修复；同一失败指纹连续出现两次就停止并报告，不自旋。
