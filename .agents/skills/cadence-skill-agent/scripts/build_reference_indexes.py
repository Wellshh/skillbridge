...
API_REFERENCE_DIRS = ("sklangref", "skipcref", "skdevref", "skoopref")
AXL_REFERENCE_DIRS = ("algroskill",)
SOURCE_VERSIONS = {
    "sklangref.pdf": "6.1.6 / November 2014",
    "sklanguser.pdf": "6.1.6 / November 2014",
    "skipcref.pdf": "6.1.6 / August 2014",
    "skdevref.pdf": "6.1.6 / August 2014",
    "skoopref.pdf": "6.1.6 / November 2014",
}
GENERATED_INDEXES = (
    ...
    for line in lines[heading_index + 1 :]:
        ...
        if candidate.startswith("```"):
            if in_code_block:
                ...
                declaration = _normalize_arrow(declaration)
...
def _render_api_index(
    entries: Sequence[ApiEntry],
    part: int,
    *,
    title: str = "SKILL API Index",
    part_label: Optional[str] = None,
    description: str = "It covers the language, IPC, development, and SKILL++ references.",
) -> str:
    ranges = {1: "A-F", 2: "G-R", 3: "S-Z and symbols"}
    label = part_label if part_label is not None else ranges[part]
    lines = [
...
def _source_records(
    ...
    for path in sorted(docs.glob("*.pdf")):
        ...
        records.append({
            "path": relative_path,
            "source_version": SOURCE_VERSIONS.get(path.name, "unknown"),
...
def _manifest(
    ...
    return {
        ...
        "indexes": {
            ...
            "api_reference_dirs": list(API_REFERENCE_DIRS),
...
def _rendered_indexes(root: Path) -> Dict[str, str]:
    ...
    axl_entries = extract_api_entries(root, AXL_REFERENCE_DIRS)
    ...
    for part, chunk in enumerate(_split_parts(axl_entries), start=1):
        rendered[f"api_index.part{part:02d}.md"] = _render_api_index(
            chunk,
            part,
            title="axl* API Index",
            part_label=f"part {part} of 3",
            description="It covers the Allegro axl* APIs documented in algroskill/.",
        )
...
def check(root: Path) -> CheckResult:
    ...
    for entry in extract_api_entries(root, AXL_REFERENCE_DIRS):
        if not (root / entry.path).is_file():
            errors.append(f"missing axl API index target: {entry.path}")
