...
def main():
 ...
 out=ROOT/'results'/'stage2-sweep-diag'/f'run-{os.getpid()}-{time.time_ns()}'; out.mkdir(parents=True); snap=ROOT/'results'/'stage0-gate'/'run-37248-1789193361092813400'; helper=snap/'stage1_smoke.il'; drc=snap/'stage0_gate.il'; context=snap/'context_probe.il'; diag=ROOT/'stage2_sweep_diag.il'; script=Path(__file__); [ (out/n).write_bytes(snapshot_source.read_bytes()) for n,snapshot_source in [('stage2_sweep_diag.py',script),('stage1_smoke.il',helper),('stage0_gate.il',drc),('context_probe.il',context),('stage2_sweep_diag.il',diag),('stage1_smoke.py',snap/'stage1_smoke.py')]]; os.chdir(out); rep={'status':'RUNNING','run_dir':str(out),'argv':sys.argv[:],'cwd':os.getcwd(),'source_sha256':hashlib.sha256(SRC.read_bytes()).hexdigest(),'cases':[]}
 for label,y,cw in CASES:
  ...
  try:
   with Allegro.open(mode='cli',board=board,workspace_id=p1,timeout=300) as opened:
    ...
    case['drc']['snapshot_before']=raw(ws,'__v2DrcSnapshot'); case['drc']['snapshot_before_fresh1']=raw(ws,'__v2DiagSnapshot'); case['drc']['snapshot_before_fresh2']=raw(ws,'__v2DiagSnapshot'); case['drc']['fresh_diag_items']=[]
    for net,rline in targets:
     da=('route',net,None,rline.layer,rline.obj_type,rline.start,rline.end,float(rline.width),rline.radius,rline.is_clockwise,rline.center); case['drc']['fresh_diag_items'] += [{'net':net,'first':raw(ws,'__v2DiagItem',*da),'second':raw(ws,'__v2DiagItem',*da)}]
    case['drc']['full_update']=raw(ws,'axlDRCUpdate',True); case['drc']['snapshot_after_original']=raw(ws,'__v2DrcSnapshot'); case['drc']['snapshot_after_fresh1']=raw(ws,'__v2DiagSnapshot'); case['drc']['snapshot_after_fresh2']=raw(ws,'__v2DiagSnapshot'); case['drc']['snapshot_after']=raw(ws,'__v2DrcSnapshot'); case['drc']['diag_snapshot_after']=raw(ws,'__v2DiagSnapshot'); case['settings']['master_after']=raw(ws,'axlDBControl',Symbol('drcEnable')); case['settings']['waived_after']=raw(ws,'axlDRCWaiveGetCount'); case['status']='CAPTURED' if len(case['creation'].get('objects',[]))==4 and case['drc'].get('snapshot_before',{}).get('ok') and case['drc'].get('snapshot_after',{}).get('ok') else 'INCOMPLETE'
