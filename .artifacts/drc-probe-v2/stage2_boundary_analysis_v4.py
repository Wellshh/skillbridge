...
ROOT=Path(__file__).resolve().parent; OUT=ROOT/'results'/'stage2-boundary'/'analysis-v4'; OUT.mkdir(parents=True,exist_ok=True); B=ROOT/'results'/'stage2-boundary'/'run-33608-1789226631112365500'; R=ROOT/'results'/'stage2-boundary-repeat'/'run-73320-1789227627605277800'
def f(x):
...
def main():
 out={'status':'OFFLINE_ANALYSIS_V4_PASS'}
 ...
 (OUT/'analysis-v4.json').write_text(json.dumps(out,indent=2),encoding='utf8'); print(json.dumps({'status':out['status'],'path':str(OUT/'analysis-v4.json')}))
