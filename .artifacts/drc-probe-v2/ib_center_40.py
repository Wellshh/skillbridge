...
BETAS=(Fraction('-0.125'),Fraction('-0.00001'),Fraction(0),Fraction('0.00001'),Fraction('0.125'))
VALUES=[(f'd{di}_s{si}_b{bi}',(dx*si,dy*si,Fraction('0.375'),be)) for di,(dx,dy) in enumerate(DIRS) for si in (1,2) for bi,be in enumerate(BETAS)]
ROLES=('arc_matrix',)
...
def call(kind, value):
    if kind=='arc_matrix':
        dx,dy,alpha,beta=value; sx,sy=Fraction('-620'),Fraction('-70'); ex,ey=sx+dx,sy+dy
        mx,my=(sx+ex)/2,(sy+ey)/2; px,py=-dy,dx; cx,cy=mx+alpha*px+beta*dx,my+alpha*py+beta*dy
        return 'arc',(float(sx),float(sy)),(float(ex),float(ey)),(float(cx),float(cy))
    v=float(value)
...
def cmd(kind, a,b,c,pad):
    ...
    return f"__v2Stage1Create('{k} \"{NET}\" \"{LAYER}\" nil {sn(s[0])}:{sn(s[1])} {sn(e[0])}:{sn(e[1])} {sn(WIDTH)} nil {ce})"
def main():
    ...
    out=ROOT/'results'/'ib-center-40'/f'run-{os.getpid()}-{time.time_ns()}'; out.mkdir(parents=True)
    source=ASSETS_DIR/'route'/'EL5_MIAN_FPC.brd'; board=Path(copy2(source,out/'EL5_MIAN_FPC.brd'))
    helper=ROOT/'stage1_smoke.il'; (out/'stage1_smoke.il').write_bytes(helper.read_bytes()); (out/'ib_center_40.py').write_bytes(Path(__file__).read_bytes()); (out/'stage1_smoke.py').write_bytes((ROOT/'stage1_smoke.py').read_bytes())
      ...
      with Allegro.open(mode='cli',board=board,workspace_id=port,timeout=300.0) as opened:
        ...
        for i,(label,v) in enumerate(VALUES):
          for role in ROLES:
            ...
            k,a,b,c=call(role,v); before=len(session.routes(net=None,layer=LAYER))+len(session.vias(net=None,layer=LAYER)); rec={'index':i,'label':label,'role':role,'H':str(v),'kind':kind,'input':{'start':[wire(a[0]),wire(a[1])],'end':[wire(b[0]),wire(b[1])],'center':[wire(c[0]),wire(c[1])]}}
            try:
              command=cmd('via',a,None,None,pad) if kind=='via' else cmd(kind,a,b,c,pad); rec['command']=command
