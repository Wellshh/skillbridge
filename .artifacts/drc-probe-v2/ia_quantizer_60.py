...
VALUES=[('zero',Fraction('0')),('ordinary',Fraction('0.1234')),('plus_01g',Fraction('0.00001')),('minus_01g',Fraction('-0.00001')),
        ('plus_09g',Fraction('0.00009')),('minus_09g',Fraction('-0.00009')),('half_even',Fraction('0.03125')),
        ('half_even_neg',Fraction('-0.03125')),('half_odd',Fraction('0.09375')),('half_odd_neg',Fraction('-0.09375'))]
...
def main():
    ...
    out=ROOT/'results'/'ia-quantizer-60'/f'run-{os.getpid()}-{time.time_ns()}'; out.mkdir(parents=True)
    source=ASSETS_DIR/'route'/'EL5_MIAN_FPC.brd'; board=Path(copy2(source,out/'EL5_MIAN_FPC.brd'))
    helper=ROOT/'stage1_smoke.il'; (out/'stage1_smoke.il').write_bytes(helper.read_bytes()); (out/'ia_quantizer_60.py').write_bytes(Path(__file__).read_bytes())
      ...
      with Allegro.open(mode='cli',board=board,workspace_id=port,timeout=300.0) as opened:
        ...
        for i,(label,v) in enumerate(VALUES):
          for role in ROLES:
            ...
            k,a,b,c=call(role,v); before=len(session.routes(net=None,layer=LAYER))+len(session.vias(net=None,layer=LAYER)); rec={'index':i*6+ROLES.index(role),'label':label,'role':role,'H':str(v),'input':wire(float(v)),'kind':kind}
            try:
        ...
        report['status']='PASS' if len(report['cases'])==60 and all(x['status']=='PASS' for x in report['cases']) else 'FAILED'
