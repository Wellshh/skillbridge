...
DIRS=((Fraction(1),Fraction(0)),(Fraction(0),Fraction(1)),(Fraction(1),Fraction(1)),(Fraction('0.7'),Fraction('1.1')))
ALPHAS=(Fraction('.25'),Fraction('.37499'),Fraction('.37501'),Fraction('.5'))
VALUES=[(f'd{di}_s{si}_a{ai}',(dx*si,dy*si,al,Fraction(0),False)) for di,(dx,dy) in enumerate(DIRS) for si in (1,2) for ai,al in enumerate(ALPHAS)]
VALUES += [(f'cw_d{di}_s{si}',(dx*si,dy*si,Fraction('.375'),Fraction('.125'),True)) for di,(dx,dy) in ((0,DIRS[0]),(3,DIRS[3])) for si in (1,2)]
ROLES=('arc_matrix',)
...
def call(kind, value):
    if kind=='arc_matrix':
        dx,dy,alpha,beta,cw=value; sx,sy=Fraction('-620'),Fraction('-70'); ex,ey=sx+dx,sy+dy
        mx,my=(sx+ex)/2,(sy+ey)/2; px,py=-dy,dx; cx,cy=mx+alpha*px+beta*dx,my+alpha*py+beta*dy
        return 'arc',(float(sx),float(sy)),(float(ex),float(ey)),(float(cx),float(cy)),cw
    v=float(value)
...
def main():
    ...
    out=ROOT/'results'/'ib-alpha-36'/f'run-{os.getpid()}-{time.time_ns()}'; out.mkdir(parents=True)
    source=ASSETS_DIR/'route'/'EL5_MIAN_FPC.brd'; board=Path(copy2(source,out/'EL5_MIAN_FPC.brd'))
    helper=ROOT/'stage1_smoke.il'; (out/'stage1_smoke.il').write_bytes(helper.read_bytes()); (out/'ib_alpha_36.py').write_bytes(Path(__file__).read_bytes()); (out/'stage1_smoke.py').write_bytes((ROOT/'stage1_smoke.py').read_bytes())
      ...
      with Allegro.open(mode='cli',board=board,workspace_id=port,timeout=300.0) as opened:
        ...
        for i,(label,v) in enumerate(VALUES):
          for role in ROLES:
            ...
            k,a,b,c,direction=call(role,v); before=len(session.routes(net=None,layer=LAYER))+len(session.vias(net=None,layer=LAYER)); dx,dy,alpha,beta,cw=v; rec={'index':i,'label':label,'role':role,'kind':kind,'direction':'cw' if cw else 'ccw','intent':{'dx':str(dx),'dy':str(dy),'alpha':str(alpha),'beta':str(beta)},'input':{'start':[wire(a[0]),wire(a[1])],'end':[wire(b[0]),wire(b[1])],'center':[wire(c[0]),wire(c[1])]}}
            try:
