...
def main():
 ...
 out=ROOT/'results'/'stage2-boundary'/f'run-{os.getpid()}-{time.time_ns()}'; out.mkdir(parents=True); snap=ROOT/'results'/'stage2-sweep-diag'/'run-63160-1789225091207869200'; helper=snap/'stage1_smoke.il'; drc=snap/'stage0_gate.il'; context=snap/'context_probe.il'; script=Path(__file__); [ (out/n).write_bytes(snapshot_source.read_bytes()) for n,snapshot_source in [('stage2_boundary.py',script),('stage1_smoke.il',helper),('stage0_gate.il',drc),('context_probe.il',context),('stage1_smoke.py',snap/'stage1_smoke.py'),('stage2_sweep_diag.il',snap/'stage2_sweep_diag.il'),('boundary_gate.py',ROOT/'boundary_gate.py')]]; os.chdir(out); rep={'status':'RUNNING','run_dir':str(out),'argv':sys.argv[:],'cwd':os.getcwd(),'source_sha256':hashlib.sha256(SRC.read_bytes()).hexdigest() ,'cases':[]}
 pending=[(f'initial-{i:02d}',y,False) for i,y in enumerate(INITIAL_CY)]; requested_bits=set(); state_by_fingerprint={}; duplicates=[]; samples=[]; stop_reason='INCOMPLETE'
 while pending and len(state_by_fingerprint)<60:
  label,y,cw=pending.pop(0)
  if bits(y) in requested_bits: continue
  ...
  try:
   with Allegro.open(mode='cli',board=board,workspace_id=p1,timeout=300) as opened:
    ...
    for net,rline in targets:
     args=('route',net,rline.layer,rline.obj_type,rline.start,rline.end,float(rline.width),rline.radius,rline.is_clockwise,rline.center); first=raw(ws,'__v2DiagItem',*args); second=raw(ws,'__v2DiagItem',*args); case['drc']['item_calls'] += [{'net':net,'first':first,'second':second,'repeat_equal':first==second}]
 ...
 rep['status']=stop_reason if stop_reason=='SEARCH_BRACKETED' else 'INCOMPLETE'; (out/'manifest.json').write_text(json.dumps(rep,indent=2,default=repr),encoding='utf8'); print(json.dumps({'status':rep['status'],'run_dir':str(out)})); return 0 if rep['status']=='SEARCH_BRACKETED' else 1
