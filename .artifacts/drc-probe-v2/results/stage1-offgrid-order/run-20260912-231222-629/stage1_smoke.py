...
def wire(value: Any) -> dict[str, str]:
    ...
    if all(ch not in text for ch in ".eE"):
        text += ".0"
    return {"%.17g": text, "bits": bits(value)}
...
def assert_requery_bits(case: str, committed: Any, row: Any) -> None:
    ...
    if case == "via":
        ...
        if float(row.rotation) != float(field(native, "rotation")):
            raise RuntimeError("via rotation differs")
        return
    ...
    if bits(float(field(native, "length17"))) != bits(float(field(native, "length"))):
        raise RuntimeError("native length17 does not round-trip")
...
def assert_echo_inputs(case: str, committed: Any, inputs: dict[str, Any]) -> None:
    echo = field(committed, "request")
    if case == "via":
        values = [("x", inputs["start"][0]), ("y", inputs["start"][1])]
    else:
        values = [("width", inputs["width"])]
        for label in ("start", "end"):
            for axis, value in zip(("x", "y"), inputs[label]):
                values.append((f"{label}.{axis}", value))
        if case == "arc":
            for axis, value in zip(("x", "y"), inputs["center"]):
                values.append((f"center.{axis}", value))
    for label, expected in values:
        key = label.rsplit(".", 1)[-1] if "." in label else label
        raw = field(echo, f"{key}17") if case == "via" else (
            field(echo, f"{key}17") if "." not in label
            else field(field(echo, label.split(".", 1)[0]), key + "17"))
        if bits(float(raw)) != expected["bits"]:
            raise RuntimeError(f"request echo mismatch for {label}")


def parent_status(returncodes: list[int], case_count: int = len(CASES)) -> str:
    return ("RUNTIME_PENDING_ALL_CASES" if len(returncodes) == case_count and all(code == 0 for code in returncodes)
            else "FAILED_RUNTIME")
...
def child(case: str, run_dir: Path) -> dict[str, Any]:
    ...
    def partial(stage: str) -> None:
        snapshot = dict(report)
        snapshot["partial_stage"] = stage
        (run_dir / "partial.json").write_text(json.dumps(snapshot, indent=2, default=str), encoding="utf-8")
    partial("before_allegro")
    ...
    with Allegro.open(mode="cli", board=board, workspace_id=port, timeout=300.0) as opened:
        ...
        owned_process = opened._runtime.process
        ...
        report["interference_baseline"] = {"routes_on_layer": len(before_routes), "vias_on_layer": len(before_vias),
                                           "exact_route_keys": [str(route_key(r)) for r in before_routes],
                                           "exact_via_locations": [str(point(v)) for v in before_vias]}
        partial("before_transaction")
        if case == "via":
            command = (f'__v2Stage1Create(\'via {skill_string(NET)} '
                       f'{skill_string(LAYER)} {skill_string(padstack)} '
                       f'{skill_num(x)}:{skill_num(y)} nil {skill_num(WIDTH)} nil nil)')
        else:
            ...
            if not report["geometry_bounds"]["end_in_extent"] or not report["geometry_bounds"]["center_in_extent"]:
                raise RuntimeError("constructed route geometry is outside paramDesign bBox")
            center_expr = f"{skill_num(center[0])}:{skill_num(center[1])}" if center else "nil"
            command = (f'__v2Stage1Create(\'{kind} {skill_string(NET)} '
                       f'{skill_string(LAYER)} nil {skill_num(x)}:{skill_num(y)} '
                       f'{skill_num(end[0])}:{skill_num(end[1])} {skill_num(WIDTH)} '
                       f'nil {center_expr})')
        ...
        assert_echo_inputs(case, committed, request_inputs)
        report["request_echo"] = jsonable(field(committed, "request"))
        partial("after_transaction")
        if case == "via":
            ...
            candidates = [r for r in rows if r.obj_type == case and route_key(r) not in before_route_keys
                          ...
                          and bits(float(r.width)) == bits(float(field(result, "width")))]
            if case == "arc":
        ...
        partial("after_requery")
        report["status"] = "PASS"
    report["owned_process_poll_after_context"] = None if owned_process is None else owned_process.poll()
    report["cleanup_observed"] = report["owned_process_poll_after_context"] is not None
    return report
...
def main() -> int:
    ...
    parent_dir = RESULTS / f"parent-{os.getpid()}-{time.time_ns()}"
    parent_dir.mkdir()
    manifest = {"status": "PENDING_RUNTIME", "cases": list(CASES), "runs": []}
    for case in CASES:
        run_dir = parent_dir / f"{case}-{len(manifest['runs'])}"
        run_dir.mkdir()
        ...
        if completed.returncode:
            ...
            break
    manifest["status"] = parent_status([r["returncode"] for r in manifest["runs"]])
    (parent_dir / "manifest.json").write_text(json.dumps(manifest, indent=2), encoding="utf-8")
    print(json.dumps(manifest, indent=2))
    return 0 if manifest["status"] == "RUNTIME_PENDING_ALL_CASES" else 1
