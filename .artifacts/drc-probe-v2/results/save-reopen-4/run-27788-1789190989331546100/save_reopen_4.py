...
def main():
    ...
    for label in CASES:
        ...
        p1=port(); p1dir=out/f"{label}-p1"; p1dir.mkdir(); os.environ["ALLEGROBRIDGE_LOG_DIRECTORY"]=str(p1dir); opened1=None
        try:
            with Allegro.open(mode="cli",board=board,workspace_id=p1,timeout=300.0) as opened1:
                ...
                smoke.assert_echo_inputs("arc",raw,row["input"]); case["creation"]["echo_assert"]=True; session.refresh()
                ...
                else: case["save"]["observable_state"]="CHANGED"
        ...
        if saved.exists() and expected is not None:
            p2=port(); p2dir=out/f"{label}-p2"; p2dir.mkdir(); os.environ["ALLEGROBRIDGE_LOG_DIRECTORY"]=str(p2dir); opened2=None
            try:
