...
2. **延迟复合批处理（`with session.batch():`）**：
   ...
   with session.batch("place decouple capacitors") as batch:
       c1 = batch.call(session.components.move, "C101", x=120.0, y=45.0, rotation=90.0)
       c2 = batch.call(session.components.move, "C102", x=125.0, y=45.0, rotation=90.0)
...
with Allegro.open(mode="manual", workspace_id="7777") as allegro:
    ...
    with pcb.batch("align filter capacitors") as batch:
        ...
        c2 = batch.call(pcb.components.move, "C102", x=125.0, y=45.0)
...
当前 M4/M5 已完成，基础对象与惰性 vias/routes/shapes 切片也已完成，并通过 Windows Allegro 实机验收。Windows DRC 探针已确认：全量 update 可接入事务，而 `axlDRCItem` 的 marker 副作用不可依赖数据库 rollback。`session.drc()`、事务写操作 `session.drc.update()` 以及仅立即执行的 `session.drc.check(component | net | pin)` 均已落地并通过 Windows Allegro 真机验收；Phase 1–6/Phase 9 验收已完成，后续仅按真实需求增加 `session.ext.rules` 或通信加固。不预先创建虚构业务 extension，也不为尚未出现的部署需求预估通信生产化工期。
...
最合理的执行顺序是：Phase 1–6/Phase 9 验收已完成；后续仅按真实需求增加 `session.ext.rules` 或通信加固。产品只支持 Allegro 17.2-2016 S048，不增加其他版本兼容分支；`drcEnable` 始终由 `unwindProtect` 显式恢复，`axlDRCItem` 不进入 transaction、preview 或 Batch。只有测试或真实使用暴露通信问题时，才进入 Phase 7 加固对应部分。
